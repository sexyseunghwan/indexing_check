...
use crate::traits::service_traits::{
    notification_service_trait::*, query_service_trait::*
};
...
impl<N: NotificationService, Q: QueryService> MainHandler<N, Q> {
    ...
    pub async fn main_task(&self, index_schedule: IndexSchedules) -> Result<(), anyhow::Error> {
        ...
        let search_index_name: String = match code_config.code_type().as_str() {
            "prod" => {
            ...
            _ => "vector-indexing-logs-2025-01-08".to_string(), /* 테스트용 */
        };
        ...
        for vector_index_log in vector_index_logs {
            ...
            if let Some(caps) = regex.captures(log_message) {
                let indexing_cnt: &str = match caps.get(0) {
                ...
                let indexing_cnt_replace: String = caps_trim.replace(",", "");
