...
async fn main_schedule_task(index_schedule: IndexSchedules) -> Result<(), anyhow::Error> {
    ...
    loop {
        ...
        if message_tick == 3 {
            ...
            match main_handler.alarm_task().await {
                Ok(_) => (),
                Err(e) => {
                    error!("[Error][main_schedule_task() -> alarm_task()] {:?}", e);
                }
            }
        }
    }
...
>>>>>>> Stashed changes
    ...
    let tasks: Vec<_> = index_schdules
        .index
        .iter()
